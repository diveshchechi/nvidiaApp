...
**************************[MAN-END]******************************************/
using namespace std;
void ReadHeader( int& w, int& h, int& d)
{
	char file[300];
	ifstream ifile;

	sprintf(file, "%s.hdr", "C:/Users/dk18/source/repos/OpenGLApp/OpenGLApp/Data/CT/CTData");
	//sprintf(file, "%s.hdr", "C:/Users/dk18/source/repos/OpenGLApp/OpenGLApp/Data/US/Volume14");
	cerr << file << endl;
	ifile.open(file);
...
void ReadVtkVolume() {
	FILE* datin0;
	datin0 = fopen("C:\\Users\\dk18\\Work\\hiwi\\files\\mushroom100\\txtFilesSeed\\test1085.txt", "r");
	for (int i = 0; i < N; i++) {
		for (int j = 0; j < N; j++) {
			for (int k = 0; k < N; k++) {
...
void ReadVolume()
{
	...
	ReadHeader( iWidth, iHeight, iDepth);
	cerr << "Volume size: [" << iWidth << "x" << iHeight << "x" << iDepth << "]" << endl;

	sprintf(file, "%s.img", "C:/Users/dk18/source/repos/OpenGLApp/OpenGLApp/Data/CT/CTData");
	//sprintf(file, "%s.hdr", "C:/Users/dk18/source/repos/OpenGLApp/OpenGLApp/Data/US/Volume14");
	int Size = iWidth * iHeight * iDepth;
...
void SetDisplayList(void)
{
	...
	glVertexPointer(3, 0x1406, 0, vPoints);
	listName = glGenLists(1);
	glNewList(listName, 0x1300);
	glDrawArrays(0x0007, 0, nElements);
	glEndList();
	delete[] vPoints;
}
